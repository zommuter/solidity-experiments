...
contract Create3Factory {
    ...
    event DeployedPrivate(address);
    event DeployedUnsafe(address);
    event EmergencyCall(address, bool, bytes);
    event EmergencyDelegateCall(address, bool, bytes);
    ...
    */
    function privateDeploy(bytes32 _salt, bytes calldata _creationCode) external payable returns (address) {
        ...
        emit DeployedPrivate(addr);
        return addr;
    }

    function _deploy(bytes32 _salt, bytes32 _salter, bytes calldata _creationCode) internal returns (address) {
    ...
    */
    ...
    function _call(address addr, bytes calldata _calldata) external returns (bool, bytes memory) {
        ...
        emit EmergencyCall(addr, success, result);
    ...
    function _delegatecall(address addr, bytes calldata _calldata) external returns (bool, bytes memory) {
        ...
        emit EmergencyDelegateCall(addr, success, result);
        return (success, result);
    }
}